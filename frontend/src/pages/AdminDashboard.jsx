...
export default function AdminDashboard({ token }) {
  ...
  const fetchDoctorKeys = async () => {
    try {
      const res = await fetch('http://localhost:5001/api/admin/doctor-keys', {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleAutoAssign = async (patientId) => {
    const res = await fetch('http://localhost:5001/api/admin/auto-assign', {
      method: 'POST',
      headers: { 
  ...
  const handleGenerateKey = async (e) => {
    ...
    const res = await fetch('http://localhost:5001/api/admin/generate-doctor-key', {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}` 
