...
export default function DoctorDashboard({ token }) {
  ...
  const fetchPatients = async () => {
    const res = await fetch('http://localhost:5001/api/doctor/patients', {
  ...
  const fetchWellness = async (id) => {
    const res = await fetch(`http://localhost:5001/api/doctor/patient/${id}/wellness`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) {
      const data = await res.json();
  ...
  const fetchAdvice = async (id) => {
    const res = await fetch(`http://localhost:5001/api/doctor/patient/${id}/advice`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) {
      const data = await res.json();
  ...
  const searchTrie = async () => {
    const res = await fetch(`http://localhost:5001/api/doctor/search?q=${searchQuery}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) {
      const data = await res.json();
  ...
  const handleUpdatePatient = async (e) => {
    e.preventDefault();
    const res = await fetch(`http://localhost:5001/api/doctor/patient/${selectedPatient.id}/update`, {
      method: 'PUT',
      headers: { 
  ...
  const handleAddAdvice = async (e) => {
    ...
    const res = await fetch('http://localhost:5001/api/doctor/advice', {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}` 
  ...
  const handleUndo = async () => {
    const res = await fetch('http://localhost:5001/api/doctor/undo-advice', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) {
