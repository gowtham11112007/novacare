...
export default function EmergencyContact({ token }) {
  ...
  const fetchContact = async () => {
    try {
      const res = await fetch("https://novacare-sccg.onrender.com/api/patient/emergency-contact", {
    ...
    <div className="card-warm p-4 bg-rose-50/70 border border-rose-200/70 rounded-2xl shadow-xs">
      ...
      {isEditing ? (
        <form onSubmit={handleSave} className="space-y-2 mt-2">
          ...
          <button
            type="submit"
            className="w-full py-1.5 bg-[#FF69B4] text-white rounded-xl text-xs font-bold"
          >
            Save Contact
          </button>
        </form>
